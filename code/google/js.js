...
$(document).ready(() => {
  $(document).on('keydown', e => {
    ...
    searchResults = searchResults || $('.g:not(.g-blk):not(#act-card):has(div > div:first > a)').not('.related-question-pair .g')
    ...
    if (activeIndex === -1) {
      ...
      const targetLink = $(targetElement).find('div > div:first > a')
      targetLink.focus()
    }
  })
})
