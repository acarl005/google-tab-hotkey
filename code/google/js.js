...
$(document).ready(() => {
  $(document).on('keydown', e => {
    ...
    searchResults = searchResults || $('.g:not(.g-blk):not(#act-card):has(.rc > .r > a, .r g-link a)').not('.related-question-pair .g')
    ...
    if (activeIndex === -1) {
      ...
      const targetLink = $(targetElement).find('.rc > .r > a, .r g-link a')
      targetLink.focus()
    }
  })
})
